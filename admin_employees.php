...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id > 0) {
        if ($password !== '') {
            $st = pdo()->prepare("UPDATE users SET employee_id=?, name=?, role=?, overtime_rate=?, password_hash=? WHERE id=?");
            $st->execute([$employee_id, $name, $role, $overtime_rate, password_hash($password, PASSWORD_DEFAULT), $id]);
        } else {
            $st = pdo()->prepare("UPDATE users SET employee_id=?, name=?, role=?, overtime_rate=? WHERE id=?");
            $st->execute([$employee_id, $name, $role, $overtime_rate, $id]);
        }
        flash_set('success', 'Pegawai diperbarui.');
    } else {
        $st = pdo()->prepare("INSERT INTO users(employee_id,name,role,password_hash,overtime_rate) VALUES(?,?,?,?,?)");
        $st->execute([$employee_id, $name, $role, password_hash($password, PASSWORD_DEFAULT), $overtime_rate]);
...
<body>
                            ...
                            <tbody>
                                <?php foreach ($rows as $r): ?>
                                    <tr>
                                        ...
                                        <td><span
                                                class="badge bg-<?= $r['role'] === 'admin' ? 'danger' : 'secondary' ?>"><?= htmlspecialchars($r['role']) ?></span>
                                        </td>
    ...
    <script>
        function fillForm(r) {
            ...
            document.getElementById('f_overtime_rate').value = r.overtime_rate;
